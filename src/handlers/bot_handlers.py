...
from models.llm import get_chatgpt_summary, get_error_message, change_model, CURRENT_MODEL, ERROR_MODEL, change_prompt
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message:
        ...
        if update.message.chat.type == 'channel':
            ...
            logger.info(f"Channel message detected. Channel ID: {channel_id}")
            
            # Always add channel and save to file when tagged
            if channel_id and channel_id not in active_channels:
                logger.info(f"Adding new channel: {channel_id}")
                active_channels.add(channel_id)
                logger.info(f"Updated active channels: {list(active_channels)}")
